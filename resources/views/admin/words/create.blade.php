...
@section('title', 'Admin.Words.Create')
...
@section('content')
  <div class="admin-content">
    ...
    @if ($errors->any())
      ...
      @endforeach        
    @endif
    <form method="post" action="{{ route('admin.words.store') }}" class="form-input admin">
      @csrf
      ...
      <div class="input-box admin" name="themes-box">
        ...
        <select class="input-text combo-box" name="theme_ids[]" id="theme_ids" @error('theme_ids[]') is-invalid @enderror multiple>
          ...
          @foreach ($themes as $theme)
            <option @if ((int) old('theme_id') === $theme->id) selected @endif value="{{ $theme->id }}">{{ $theme->id }}&emsp;{{ $theme->title }}</option>
          @endforeach
        </select>
